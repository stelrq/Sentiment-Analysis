...
def create_tokenizer(lines):
    ...
    return tokenizer
# define the model
def define_model(n_words, neuron_count):
    # define network
    model = Sequential()
    model.add(Dense(neuron_count, input_shape=(n_words,), activation='relu'))
...
def train_model(train_docs, test_docs, mode):
    ...
    return Xtrain, Xtest
def evaluate_model(Xtrain, Xtest, ytrain, ytest, neuron_count):
    scores = list();
    n_words = Xtest.shape[1]
    n_runs = 10
    for i in range(n_runs):
        #initialize model
        model = define_model(n_words, neuron_count)
        # fit network
...
##evaluate modes
#evaluate_modes(modes, train_docs, test_docs, ytrain, ytest)
#define neuron counts
neurons = [50, 40, 30, 20, 10]
evaluate_hidden_layer(neurons, train_docs, test_docs, ytrain, ytest)