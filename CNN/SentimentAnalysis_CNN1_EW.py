...
# load the model
model = load_model('model5pass48filter.h5')
...
# test positive text
text = 'T: Warren is a hard hitting civil rights warrior with a strong policy based campaign.'
percent, sentiment = predict_sentiment(text, vocab, tokenizer, max_length, model)
print('Review: [%s]\nSentiment: %s (%.3f%%)'% (text, sentiment, percent*100))
# test negative text
text = 'T: Elizabeth Warren is a liar that will say anything to further her own ends!'
percent, sentiment = predict_sentiment(text, vocab, tokenizer, max_length, model)
print('Review: [%s]\nSentiment: %s (%.3f%%)'% (text, sentiment, percent*100))